...
@app.route('/todo_apps/top/AI')
def AI_predict():
    try:
        #DBからデータを取ってAIに入れてDBに返す作業を入れる
        # タスク一覧を取得
        todos = business_logic.find_todo_AI_all(session["u_id"])
        return render_template("AI_pred.html", todos=todos)
    except:
        flash('エラー', 'error')
        return redirect('/todo_apps/top')
    finally:
        try:
            session.close()
        except:
...
@app.route('/todo_apps/top/wait')
def AI_predict_wait():
    time.sleep(6)
    return redirect('/todo_apps/top/AI')
